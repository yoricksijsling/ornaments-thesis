...
open Relation.Binary.PropositionalEquality.Deprecated-inspect-on-steroids hiding ([_])
...
-- traverseE elab xs nm = traverseL (λ a → elab a nm) xs

-- -- open RawMonadZero ⦃ ... ⦄
-- -- open module nono {f} {M : Set f → Set f}
-- --                     ⦃ Mon : RawMonadZero M ⦄ = RawMonadZero Mon

-- -- open module nono {M : Set → Set} ⦃ Mon : RawMonadZero M ⦄
-- --   = RawMonadZero (elabMonZero)

-- open RawMonadZero elabMonZero
...
-- --       parseParams t >>= λ n → 
-- --       return (1 + n)
-- --     -- where open RawMonad elabMon
-- --   parseParamsT (sort x) 
-- --     = -- End of telescope 
-- --       return 0
-- --     -- where open RawMonad elabMon
-- --   parseParamsT _ = ∅
-- --     -- where open RawMonadZero elabMonZero
