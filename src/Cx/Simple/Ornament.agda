...
infixr 3 _⊕_
infixr 4 -⊗_ rec-⊗_ insert_⊗_ insert-rec-⊗_
data ConOrn : ∀{Γ Δ} (f : Cxf Δ Γ) (D : ConDesc Γ) → Set₂ where
  ...
  insert_⊗_ : ∀{Γ Δ xs}{c : Cxf Δ Γ} →
             (S : (δ : ⟦ Δ ⟧) → Set) → (xs⁺ : ConOrn (cxf-forget c S) xs) → ConOrn c xs
  insert-rec-⊗_ : ∀{Γ Δ xs}{c : Cxf Δ Γ} → (xs⁺ : ConOrn c xs) → ConOrn c xs
...
conOrnToDesc (insert_⊗_ S xs⁺) = S ⊗ (conOrnToDesc xs⁺)
conOrnToDesc (insert-rec-⊗_ xs⁺) = rec-⊗ (conOrnToDesc xs⁺)
...
conForgetNT (insert_⊗_ S xs⁺) (s , v) = conForgetNT xs⁺ v
conForgetNT (insert-rec-⊗_ xs⁺) (s , v) = conForgetNT xs⁺ v
