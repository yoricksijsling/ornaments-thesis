...
data Orn {I J}(u : Cxf J I) : ∀{Γ Δ dt} (c : Cxf Δ Γ) (D : Desc I Γ dt) → Set₁ where
  ι : ∀{Γ Δ i}{c : Cxf Δ Γ} → (j : (δ : ⟦ Δ ⟧) → cxf-Inv u (i (c $$ δ))) → Orn u c (ι i)
  -⊗_ : ∀{Γ Δ S xs}{c : Cxf Δ Γ} → (xs⁺ : Orn u (cxf-both c) xs) → Orn u c (S ⊗ xs)
  rec_⊗_ : ∀{Γ Δ i xs}{c : Cxf Δ Γ} →
            (j : (δ : ⟦ Δ ⟧) → cxf-Inv u (i (c $$ δ))) → (xs⁺ : Orn u c xs) → Orn u c (rec i ⊗ xs)
...
module _ {I J}{u : Cxf J I} where
  ...
  ornToDesc (-⊗_ {S = S} {c = c} xs⁺) = S ∘ _$$_ c ⊗ ornToDesc xs⁺
...
idOrn : ∀{I Γ dt}{D : Desc I Γ dt} → Orn (cxf-id I) (cxf-id Γ) D
...
module _ {I J}{u : Cxf J I} where
  forgetNT : ∀{Γ Δ dt}{c : Cxf Δ Γ}{D : Desc I Γ dt} (o : Orn u c D) →
             ∀ {δ X j} → ⟦ o ⟧ δ (X ∘ _$$_ u) j → ⟦ D ⟧ (c $$ δ) X (u $$ j)
