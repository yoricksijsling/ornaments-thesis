...
module _ {I : Set}{J : I → Set} where
  ...
  algOrn {dt = isCon} {c = c} (nm /rec i ⊗ xs) α = insert "_" / (J ∘ i ∘ apply c)
...
module _ {I J K : Set}{u : J → I}{v : K → J} where
  ...
  compose (ι j) (insert nm / T ⊗ ys⁺) = insert nm / T ⊗ (compose (ι j) ys⁺)
  compose (ι j) (insert nm /rec k ⊗ ys⁺) = insert nm /rec k ⊗ (compose (ι j) ys⁺)
  compose (-⊗ xs⁺) (-⊗ ys⁺) = -⊗ (compose xs⁺ ys⁺)
  compose (-⊗ xs⁺) (insert nm / T ⊗ ys⁺) = insert nm / T ⊗ (compose (-⊗ xs⁺) ys⁺)
  compose (-⊗ xs⁺) (insert nm /rec j ⊗ ys⁺) = insert nm /rec j ⊗ (compose (-⊗ xs⁺) ys⁺)
  compose (-⊗ xs⁺) (give-K t ys⁺) = give-K t (compose xs⁺ ys⁺)
  compose (rec j ⊗ xs⁺) (rec k ⊗ ys⁺) = rec (λ _ → inv-∘ (j _) (k _)) ⊗ compose xs⁺ ys⁺
  compose (rec j ⊗ xs⁺) (insert nm / T ⊗ ys⁺) = insert nm / T ⊗ (compose (rec j ⊗ xs⁺) ys⁺)
  compose (rec j ⊗ xs⁺) (insert nm /rec k ⊗ ys⁺) = insert nm /rec k ⊗ (compose (rec j ⊗ xs⁺) ys⁺)
  compose {d = d} (insert nm / S ⊗ xs⁺) (-⊗ ys⁺) = insert nm / (S ∘ apply d) ⊗ (compose xs⁺ ys⁺)
  compose (insert nm / S ⊗ xs⁺) (insert nm2 / T ⊗ ys⁺) = insert nm2 / T ⊗ (compose (insert nm / S ⊗ xs⁺) ys⁺)
  compose (insert nm / S ⊗ xs⁺) (insert nm2 /rec k ⊗ ys⁺) = insert nm2 /rec k ⊗ (compose (insert nm / S ⊗ xs⁺) ys⁺)
  compose (insert nm / S ⊗ xs⁺) (give-K t ys⁺) = compose xs⁺ ys⁺ -- This can't be rightnmnm
  compose (insert nm /rec j ⊗ xs⁺) (rec k ⊗ ys⁺) = insert nm /rec (uninv ∘ k) ⊗ (compose xs⁺ ys⁺)
  compose (insert nm /rec j ⊗ xs⁺) (insert nm2 / T ⊗ ys⁺) = insert nm2 / T ⊗ (compose (insert nm /rec j ⊗ xs⁺) ys⁺)
  compose (insert nm /rec j ⊗ xs⁺) (insert nm2 /rec k ⊗ ys⁺) = insert nm2 /rec k ⊗ (compose (insert nm /rec j ⊗ xs⁺) ys⁺)
  ...
  module _ (ext : ∀{a b} → Extensionality a b) where
    ...
    compose-sound (ι {i = i} {c} j) (insert nm / T ⊗ ys⁺) = cong (_/_⊗_ nm T) (compose-sound (ι {i = i} {c} j) ys⁺)
    compose-sound (ι {i = i} {c} j) (insert nm /rec k ⊗ ys⁺) = cong (_/rec_⊗_ nm k) (compose-sound (ι {i = i} {c} j) ys⁺)
    compose-sound (-⊗ xs⁺) (-⊗ ys⁺) = cong (_/_⊗_ _ _) (compose-sound xs⁺ ys⁺)
    compose-sound (-⊗ xs⁺) (insert nm / T ⊗ ys⁺) = cong (_/_⊗_ nm T) (compose-sound (-⊗ xs⁺) ys⁺)
    compose-sound (-⊗ xs⁺) (insert nm /rec k ⊗ ys⁺) = cong (_/rec_⊗_ nm k) (compose-sound (-⊗ xs⁺) ys⁺)
    ...
    compose-sound (rec_⊗_ {i = i} j xs⁺) (insert nm / T ⊗ ys⁺) = cong (_/_⊗_ nm T) (compose-sound (rec_⊗_ {i = i} j xs⁺) ys⁺)
    compose-sound (rec_⊗_ {i = i} j xs⁺) (insert nm /rec k ⊗ ys⁺) = cong (_/rec_⊗_ nm k) (compose-sound (rec_⊗_ {i = i} j xs⁺) ys⁺)
    compose-sound {d = d} (insert nm / S ⊗ xs⁺) (-⊗ ys⁺) = cong (_/_⊗_ nm (S ∘ apply d)) (compose-sound xs⁺ ys⁺)
    compose-sound (insert nm / S ⊗ xs⁺) (insert nm2 / T ⊗ ys⁺) = cong (_/_⊗_ nm2 T) (compose-sound (insert nm / S ⊗ xs⁺) ys⁺)
    compose-sound (insert nm / S ⊗ xs⁺) (insert nm2 /rec k ⊗ ys⁺) = cong (_/rec_⊗_ nm2 k) (compose-sound (insert nm / S ⊗ xs⁺) ys⁺)
    compose-sound (insert nm / S ⊗ xs⁺) (give-K t ys⁺) = compose-sound xs⁺ ys⁺
    compose-sound (insert nm /rec j ⊗ xs⁺) (rec k ⊗ ys⁺) = cong (_/rec_⊗_ nm (uninv ∘ k)) (compose-sound xs⁺ ys⁺)
    compose-sound (insert nm /rec j ⊗ xs⁺) (insert nm2 / T ⊗ ys⁺) = cong (_/_⊗_ nm2 T) (compose-sound (insert nm /rec j ⊗ xs⁺) ys⁺)
    compose-sound (insert nm /rec j ⊗ xs⁺) (insert nm2 /rec k ⊗ ys⁺) = cong (_/rec_⊗_ nm2 k) (compose-sound (insert nm /rec j ⊗ xs⁺) ys⁺)
