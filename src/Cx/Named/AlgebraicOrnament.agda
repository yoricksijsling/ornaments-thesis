...
module _ {I : Set}{J : I → Set} where
  ...
  algOrn {dt = isCon} {c = c} (nm /rec i ⊗ xs) α = "_" / (J ∘ i ∘ apply c)
                                                +⊗ nm /rec (λ δ → inv (i (apply c (pop δ)) , top δ))
...
module _ {I J K : Set}{u : J → I}{v : K → J} where
  ...
  compose {d = d} (nm′ / S +⊗ xs⁺) (nm /-⊗ ys⁺) = nm / (S ∘ apply d) +⊗ (compose xs⁺ ys⁺)
  ...
  module _ (ext : ∀{a b} → Extensionality a b) where
    ...
    compose-sound {d = d} (nm′ / S +⊗ xs⁺) (nm /-⊗ ys⁺) = cong (_/_⊗_ nm (S ∘ apply d)) (compose-sound xs⁺ ys⁺)
