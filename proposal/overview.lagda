...
functions for different datatypes.
...
programmers better.
They provide insight into how datatypes relate to each other, which
seems fundamental for future work on functional programming languages.

In current implementations, users have to convert their datatypes to
...
\item We aim to be able to generate new datatypes using ornaments,
...
\item Implement a library for generic programming in Agda.
  It must be able to take an existing datatype and lift it to the
